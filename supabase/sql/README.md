...
| 13 | `v5_rooms.sql` | EN-44 / KK-2 — a private battle for 3–4 friends | The group-battle screen shows "әлі қосылмаған"; friend battles stay strictly 1v1. **Also adds `rooms` and `room_members` to the realtime publication** — the lobby is worthless without it |
| 12 | `v5_cosmetics_content.sql` | EN-41 / KK-6 — cosmetics worth wanting | The new frame effects work on any item that has them, but no item does; the shop keeps its existing flat rings. **Run after #4** — prices are already on the coin scale |
